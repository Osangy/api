...
function queueFile(fileUrl, mid, position) {
  getTopic((err, topic) => {
    if (err) {
      logging.error('Error occurred while getting pubsub topic', err);
      return;
...
function queueApiAiResponse(response){
  ...
  getTopic((err, topic) => {
    ...
    topic.publish({
      aiResponse : stringResponse
    }, (err) => {
      if (err) {
        logging.error('Error occurred while queuing background task', err);
      } else {
        logging.info(`API.ai response queued for background processing`);
      }
    });
  });
}
